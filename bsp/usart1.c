...
void USART1_IRQHandler(void)
{
	...
	if(USART_GetITStatus(USART1, USART_IT_IDLE) != RESET)
	{
		...
		if(this_time_rx_len == RC_FRAME_LEN)
		{
			...
			{
				memcpy(relay_buf+1, usart1_dma_rx_buf[0], RELAY_BUF_SIZE-1);
				DBUS_DEC(&dbus, usart1_dma_rx_buf[0]);
				DMA2_Stream2->CR |= (uint32_t)(DMA_SxCR_CT);        //enable the current selected memory is Memory 1
			}
			else
			{
				memcpy(relay_buf+1, usart1_dma_rx_buf[1], RELAY_BUF_SIZE-1);
				DBUS_DEC(&dbus, usart1_dma_rx_buf[1]);
